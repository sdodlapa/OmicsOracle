...
async def demo_1_basic_caching():
    ...
    sample_content = {
        ...
        "sections": [
            ...
            {"heading": "Results", "text": "Our model achieved 95% accuracy...", "paragraphs": 18},
        ],
        "tables": [
            {
            ...
            },
            {
                ...
                "rows": 5,
                "columns": 3,
                "data": [["Dataset", "Samples", "Features"], ["Training", "8000", "23000"]],
            },
        ],
        "figures": [
            {"caption": "Figure 1: Model Architecture", "type": "diagram", "url": "/figures/fig1.png"},
            {"caption": "Figure 2: Performance Comparison", "type": "plot", "url": "/figures/fig2.png"},
        ],
        "references": [
    ...
    print(
        f"  - Content: {len(sample_content['sections'])} sections, "
...
async def demo_3_compression_comparison():
    ...
    large_content = {
        ...
        "tables": [
            {"caption": f"Table {i}", "data": [[f"Cell-{i}-{j}-{k}" for k in range(20)] for j in range(50)]}
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        cache_uncompressed = ParsedCache(cache_dir=Path(tmpdir) / "uncompressed", use_compression=False)
        ...
        cache_compressed = ParsedCache(cache_dir=Path(tmpdir) / "compressed", use_compression=True)
...
async def demo_6_real_world_simulation():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        for paper_id in paper_ids:
            await cache.save(paper_id, {"title": f"Paper {paper_id}", "tables": []}, parse_duration_ms=2000)

...
async def test_query(query: str, description: str, max_datasets: int = 3):
    ...
    try:
        result = await pipeline.collect(query=query, max_datasets=max_datasets, max_citing_papers=30)
        ...
        logger.info(
            f"  {'✅' if success['found_citations'] else '⚠️ '} Found citations: {success['found_citations']}"
        )
...
async def main():
    ...
    test_cases = [
        ...
        {"query": "mouse liver ChIP-seq", "description": "Mouse liver ChIP sequencing", "max_datasets": 2},
        ...
        {"query": "TP53 mutation cancer", "description": "TP53 mutations in cancer", "max_datasets": 2},
    ]
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        results.append({"test": test_case["description"], "query": test_case["query"], **result})
    ...
    for i, result in enumerate(results, 1):
        ...
        logger.info(
            f"   Datasets: {result['found_datasets']}, Citations: {result['found_citations']}, Full-text: {result['got_fulltext']}"
        )
        if "error" in result:

...
def test_scholar_search():
    ...
    try:
        # Create config
        config = GoogleScholarConfig(enable=True, max_results=50, rate_limit_seconds=3.0)
...
def test_scholar_year_range():
    ...
    try:
        ...
        results = client.search(query, max_results=5, year_from=year_from, year_to=year_to)

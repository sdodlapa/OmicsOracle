...
async def demonstrate_robust_search():
    ...
    for i, research in enumerate(RESEARCH_QUERIES, 1):
        ...
        try:
            ...
            with_fulltext = sum(1 for p in results.publications if p.metadata and p.metadata.get("fulltext_url"))
            with_citations = sum(1 for p in results.publications if p.metadata and p.metadata.get("citation_count"))
            ...
            fulltext_papers = [p for p in results.publications if p.metadata and p.metadata.get("fulltext_url")]
            if fulltext_papers:

...
def scholar_config():
    ...
    return GoogleScholarConfig(
        enable=True,
        max_results=50,
        rate_limit_seconds=0.5,  # Minimum allowed
        use_proxy=False
    )
...
class TestScholarClientInitialization:
    ...
    def test_init_without_scholarly_library(self, scholar_config, monkeypatch):
        ...
        monkeypatch.setattr(
            'omics_oracle_v2.lib.publications.clients.scholar.SCHOLARLY_AVAILABLE',
            False
        )
...
class TestScholarSearch:
    ...
    def test_search_applies_rate_limiting(self, mock_sleep, mock_scholarly, scholar_config, mock_scholar_result):

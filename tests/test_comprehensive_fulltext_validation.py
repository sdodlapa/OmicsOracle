...
spec = importlib.util.spec_from_file_location(
    "diverse_papers",
    Path(__file__).parent / "test_datasets" / "100_diverse_papers.py"
)
...
async def validate_comprehensive_fulltext():
    ...
    async with FullTextManager(config) as manager:
        ...
        for i in range(0, len(publications), batch_size):
            ...
            print(f"  Batch results: {success_count}/{len(batch)} found ({success_count/len(batch)*100:.1f}%)")
        ...
        if publisher_stats:
            ...
            print("-"*80)
            for publisher, stats in sorted(publisher_stats.items(), key=lambda x: x[1]["total"], reverse=True):
                percentage = stats["found"] / stats["total"] * 100 if stats["total"] > 0 else 0

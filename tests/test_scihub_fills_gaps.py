...
async def test_scihub_fills_gaps():
    ...
    print("RESULTS")
    print("=" * 80)
    print(f"Phase 1 (Legal OA):      {phase1_found}/{len(test_papers)} ({phase1_found/len(test_papers)*100:.1f}%)")
    print(f"Phase 2 (+ Sci-Hub):     {phase2_found}/{len(test_papers)} ({phase2_found/len(test_papers)*100:.1f}%)")
    print(f"Sci-Hub filled gaps:     {scihub_count}/{len(test_papers)} ({scihub_count/len(test_papers)*100:.1f}%)")

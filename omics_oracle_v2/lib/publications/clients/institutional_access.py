...
class InstitutionalConfig:
    ...
    fallback_methods: List[str] = field(default_factory=lambda: [
        "unpaywall", "ezproxy", "openurl", "direct"
    ])
...
class InstitutionalAccessManager:
    ...
    def __init__(
        ...
        logger.info(
            f"InstitutionalAccessManager initialized for {institution.value}"
        )
    ...
    def get_access_url(
        ...
        if prefer_method:
            methods = [prefer_method] + [
                m for m in self.config.fallback_methods if m != prefer_method
            ]
        else:
        ...
        for method in methods:
            try:
                ...
                if url:
                    logger.info(
                        f"Found access via {method}: {url[:100]}..."
                    )
                    return url
        ...
        logger.warning(
            f"No access method worked for {publication.title[:50]}..."
        )
        return None
    ...
    def get_access_instructions(
        ...
        unpaywall_url = self._try_unpaywall(publication)
        if unpaywall_url:
            instructions['open_access'] = (
                f"✅ Free open access version available: {unpaywall_url}"
            )
        ...
        if self.config.openurl_resolver:
            ...
            if openurl:
                instructions['link_resolver'] = (
                    f"🔗 Find full text via library resolver: {openurl}"
                )
...
def create_institutional_manager(
    ...
    """
    inst_type = InstitutionType.GEORGIA_TECH if institution.lower() == "gatech" else InstitutionType.OLD_DOMINION

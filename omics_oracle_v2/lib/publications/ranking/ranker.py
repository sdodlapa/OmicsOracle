...
class PublicationRanker:
    ...
    def __init__(self, config: PublicationSearchConfig):
        ...
        if not (0.99 <= sum(self.weights.values()) <= 1.01):
            logger.warning(
                f"Ranking weights sum to {sum(self.weights.values())}, "
                "should be 1.0"
            )
    ...
    def rank(
        ...
        for pub in publications:
            score, breakdown, matches = self._score_publication(
                pub, query, query_tokens
            )
    ...
    def _score_publication(
        ...
        title_score = self._calculate_text_relevance(
            publication.title or "", query_tokens, matches
        )
        ...
        abstract_score = self._calculate_text_relevance(
            publication.abstract or "", query_tokens, matches
        )
    ...
    def _tokenize(self, text: str) -> Set[str]:
        ...
        stop_words = {
            ...
            'might', 'must', 'can', 'of', 'for', 'in', 'to', 'and', 'or'
        }

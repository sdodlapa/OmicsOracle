...
from omics_oracle_v2.lib.publications.pdf_download_manager import (
    PDFDownloadManager,
    DownloadReport
)
...
class GEOCitationPipeline:
    ...
    async def collect(
        ...
        max_citing_papers: Optional[int] = None
    ) -> CollectionResult:
        """
        ...
        search_result = await self.geo_client.search(
            optimized_query,  # Use optimized query
            max_results=max_geo
        )
        ...
        datasets_dict = await self.geo_client.batch_get_metadata(
        ...
        papers_with_fulltext = await self.fulltext_manager.get_fulltext_batch(
            unique_papers
        )
        ...
        logger.info(f"  Full-text coverage: {fulltext_coverage:.1%} ({fulltext_count}/{len(papers_with_fulltext)})")

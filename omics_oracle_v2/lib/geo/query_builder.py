...
STOP_WORDS = {
    ...
    'data', 'dataset', 'datasets', 'analysis', 'study', 'studies', 'using', 'based',
    'joint', 'profiling', 'integrated', 'combined', 'multi', 'comprehensive'
}
...
class GEOQueryBuilder:
            ...
            if add_synonyms:
                for tech_name, synonyms in self.technique_synonyms.items():
                    if keyword in tech_name or any(keyword in syn.lower() for syn in synonyms):
                        ...
                        field_synonyms = [
                            f'{syn}[Title]' if ' ' not in syn else f'"{syn}"[Title]'
    ...
    def _is_scientific_concept(self, phrase: str) -> bool:
        ...
        """
        scientific_concepts = {
            'dna methylation', 'rna sequencing', 'gene expression',
            'chromatin accessibility', 'histone modification', 
            'transcription factor', 'cell differentiation', 'genome organization',
            'protein binding', 'dna binding', 'epigenetic regulation',
            'chromosome conformation', 'chromatin structure'
        }

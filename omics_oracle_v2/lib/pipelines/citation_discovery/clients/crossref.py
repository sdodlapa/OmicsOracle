...
class CrossrefClient:
    ...
    def get_citing_papers(
        ...
        try:
            ...
            logger.info(f"DOI {clean_doi} has {citation_count} citations (per Crossref metadata)")
    ...
    def search(self, query: str, limit: int = 20) -> List[Publication]:
        ...
        try:
            ...
            logger.info(f"✓ Crossref search: found {len(papers)} papers for query '{query}'")
            return papers
    ...
    def _parse_result(self, item: Dict[str, Any]) -> Optional[Publication]:
        ...
        try:
            ...
            pub_date = None
            date_parts = item.get('published', item.get('created', {})).get('date-parts', [[]])
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.error(f"Crossref request failed after {max_retries} attempts: {e}")
                    return None

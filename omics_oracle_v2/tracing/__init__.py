...
class RequestTracer:
    ...
    def start_trace(cls, query: str, workflow_type: str, user_id: Optional[str] = None) -> str:
        ...
        trace = RequestTrace(query=query, workflow_type=workflow_type, user_id=user_id)
    ...
    def add_event(cls, trace_id: str, event: TraceEvent):
        ...
        if trace_id not in cls._traces:
            ...
            cls._traces[trace_id] = RequestTrace(trace_id=trace_id, query="Unknown", workflow_type="Unknown")
    ...
    def clear_old_traces(cls, max_age_hours: int = 24):
        ...
        to_delete = [
            trace_id for trace_id, trace in cls._traces.items() if trace.started_at.timestamp() < cutoff
        ]

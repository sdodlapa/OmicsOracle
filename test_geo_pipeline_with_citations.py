    ...
    for query in queries:
        ...
        if search_result.geo_ids:
            ...
            metadata_dict = await geo_client.batch_get_metadata(
                search_result.geo_ids[:5],  # Check first 5
                max_concurrent=3
            )
...
async def test_with_citations():
    ...
    logger.info(f"{'✅' if len(result.datasets_found) > 0 else '❌'} Found {len(result.datasets_found)} GEO datasets")
    logger.info(f"{'✅' if result.total_citing_papers > 0 else '⚠️ '} Found {result.total_citing_papers} citing papers")
    logger.info(f"{'✅' if result.fulltext_coverage > 0 else '⚠️ '} Got {result.fulltext_coverage:.1f}% full-text coverage")

...
async def test_auto_discovery_flow():
    ...
    try:
        ...
        if existing_data:
            print(f"[WARN]  {test_geo_id} already exists in database")
            print(
                f"   Citations: {len(existing_data.get('papers', {}).get('original', []))}"
            )
            print("   Test will use cached data (no auto-discovery)")
        else:
            print(f"[OK] {test_geo_id} not in database - will trigger auto-discovery")
        print()
        ...
        print(f"[TIME]  Completed in {elapsed_ms:.2f}ms")
        print()
        ...
        if geo_data is None:
            print("[FAIL] FAILED: geo_data is None")
            return False

        print("[OK] Got geo_data")
        print()
        ...
        if citation_count == 0:
            print(
                "[WARN]  WARNING: No citations found (citation discovery may have failed)"
            )
            print("   This is acceptable for datasets with no citing papers")
        else:
            print(f"[OK] Found {citation_count} citations")
            print()
        ...
        print(f"[TIME]  Cache hit in {cache_elapsed_ms:.2f}ms")

        if cache_elapsed_ms < 100:  # Should be <100ms for cache hit
            print(f"[OK] Fast cache retrieval ({cache_elapsed_ms:.2f}ms < 100ms)")
        else:
            print(f"[WARN]  Slow cache retrieval ({cache_elapsed_ms:.2f}ms)")
        print()
        ...
        print("TEST SUMMARY")
        print("=" * 80)
        print(f"[OK] Auto-discovery flow: WORKING")
        print(
            f"[OK] GEO metadata: {geo_data.get('geo', {}).get('title', 'MISSING')[:50]}"
        )
        print(f"[OK] Citations: {citation_count} found")
        print(f"[OK] Cache performance: {cache_elapsed_ms:.2f}ms")
        print()
...
async def test_search_service_integration():
    ...
    try:
        ...
        request = SearchRequest(
            search_terms=["GSE189158"], max_results=10, enable_semantic=False
        )
        ...
        elapsed_ms = (end_time - start_time) * 1000
        print(f"[TIME]  Search completed in {elapsed_ms:.2f}ms")
        print()
        ...
        if len(result.datasets) > 0:
            ...
            if dataset.citation_count > 0:
                print("[OK] Auto-discovery enrichment: WORKING")
            else:
                print("[WARN]  No citations (may need manual discovery)")
        else:
            print("[FAIL] No datasets found")
            return False

        print("SEARCH SERVICE TEST: PASSED [OK]")
        return True

    except Exception as e:
        print(f"\n[FAIL] SEARCH SERVICE TEST FAILED: {e}")
...
async def main():
    ...
    print("FINAL SUMMARY")
    print("=" * 80)
    print(f"Auto-Discovery Flow: {'[OK] PASSED' if test1_passed else '[FAIL] FAILED'}")
    print(
        f"Search Service Integration: {'[OK] PASSED' if test2_passed else '[FAIL] FAILED'}"
    )
    print("=" * 80)
